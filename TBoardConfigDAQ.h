...
private:
  ...
  int fCurrentLimitAnalogue;
    ...
    std::uint32_t fAutoShutOffDelay;
...
protected:
     ...
     *  subsequent triggers.
     */
    static const std::uint32_t STROBE_DELAY;
...
public:
  ...
  std::uint32_t GetAutoShutOffDelay(){return fAutoShutOffDelay;};
  ...
  // TRIGGER Module
  std::uint32_t GetBusyDuration()    {return fBusyDuration;};
  int GetTriggerMode()          {return fTriggerMode;};
  int GetStrobeDuration()       {return fStrobeDuration;};
  std::uint32_t GetStrobeDelay()     {return fStrobeDelay;};
  ...
  void SetStrobeDelay (std::uint32_t delay)     {fStrobeDelay        = delay;};
